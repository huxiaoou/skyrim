...
class CManagerLibReader(CMangerLibBase):
    ...
    def read_by_conditions(self, t_conditions: List[tuple],
                           t_value_columns: List[str],
                           t_using_default_table: bool = True, t_table_name: str = ""):
        """

        :param t_conditions: a list of tuple, like:[
                            ("instrument", "=", "IC.CFE"),
                            ("tid", "=", "T01"),
                            ("trade_date", ">=", "20120101"),
                            ("trade_date", "<", "20120101"),
        ...
        _table_name = self.m_default_table if t_using_default_table else t_table_name
        str_value_columns = ", ".join(t_value_columns)
        _conds_str = " and ".join(["{} {} '{}'".format(c0, c1, c2) for c0, c1, c2 in t_conditions])
        cmd_sql_for_inquiry = "SELECT {} FROM {} where {}".format(
            str_value_columns, _table_name, _conds_str)
        rows = self.m_cursor.execute(cmd_sql_for_inquiry).fetchall()
        t_df = pd.DataFrame(data=rows, columns=t_value_columns)
        return t_df
